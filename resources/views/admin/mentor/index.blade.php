    ...
    <div class="bg-white/5 border border-white/10 rounded-sm overflow-hidden shadow-2xl">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-white/5 border-b border-white/10 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                        ...
                        <tr class="group hover:bg-white/2 transition-colors">
                            ...
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-4">
                                    <div class="h-12 w-12 rounded-sm overflow-hidden border border-white/10 bg-slate-800 shrink-0">
                                        <img src="{{ asset('storage/' . ($mentor->photo_path ?? $mentor->photo)) }}" class="h-full w-full object-cover">
                                    </div>
                                    <div class="flex flex-col">
                                        <span class="text-sm font-black text-white uppercase tracking-tight">{{ $mentor->name }}</span>
                                        <span class="text-[10px] font-bold text-blue-500 uppercase tracking-widest">{{ $mentor->specialty ?? $mentor->specialization }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-5 text-center">
                                @if($mentor->expertise_badge)
                                    <span class="bg-fiesphere-yellow text-fiesphere-blue px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-tighter shadow-sm">
                            ...
                            {{-- Order Index --}}
                            <td class="px-6 py-5 text-center">
                                <span class="text-[11px] font-black text-white bg-white/5 w-8 h-8 inline-flex items-center justify-center rounded-xl border border-white/5">
                                    {{ $mentor->order ?? $mentor->order_index }}
                                </span>
                            </td>

                            {{-- Status --}}
                            <td class="px-6 py-5 text-center">
                                <span class="h-2 w-2 inline-block rounded-full {{ $mentor->is_active ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-red-500' }}"></span>
                            </td>
                            ...
                            <td class="px-8 py-5 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    {{-- Tombol Edit --}}
                                    <a href="{{ route('admin.mentor.edit', $mentor->id) }}" class="p-3 bg-white/5 hover:bg-blue-600 text-white rounded-xl transition-all group/btn">
                                    ...
                                    <flux:modal.trigger name="delete-mentor-{{ $mentor->id }}">
                                        <button class="p-3 bg-white/5 text-red-400 hover:bg-red-500 hover:text-white rounded-xl transition-all">
                                    ...
                                    <form action="{{ route('admin.mentor.destroy', $mentor->id) }}" method="POST">
                                        ...
                                        <div class="text-center space-y-6">
                                            ...
                                            <div class="space-y-2">
                                                ...
                                                <p class="text-sm text-slate-400">Data mentor <span class="text-white font-bold">"{{ $mentor->name }}"</span> akan dihapus permanen dari sistem.</p>
                                            </div>
