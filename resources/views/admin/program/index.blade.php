<x-layouts.admin title="Manajemen Program & Harga">
    <div class="mb-10 flex items-end justify-between">
        <div class="space-y-1">
            <h3 class="text-lg font-bold text-slate-400  tracking-widest">Daftar Paket Kursus Fiesphere</h3>
                ...
                <tbody class="divide-y divide-white/5">
                    @forelse($programs as $item)
                        <tr class="hover:bg-white/2 transition-colors group">
                            <td class="px-8 py-6">
                                <span class="text-lg font-black text-white  tracking-tight">{{ $item->title }}</span>
                            </td>
                            <td class="px-8 py-6">
                            ...
                            <td class="px-8 py-6">
                                @if($item->is_featured)
                                    <span class="px-4 py-1 rounded-md bg-blue-600/20 border border-blue-500/30 text-[10px] font-black text-blue-400 text-md tracking-widest">Recommended</span>
                                @else
                                    <span class="px-4 py-1 rounded-md bg-white/5 border border-white/10 text-[10px] font-black text-slate-500  tracking-widest">Standard</span>
                                @endif
                            </td>
                            <td class="px-8 py-6">
                        ...
                        <tr>
                            <td colspan="5" class="px-8 py-20 text-center">
                                <div class="flex flex-col items-center opacity-20">
                                    ...
                                    <p class="font-black  tracking-widest">Belum ada paket program</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <flux:modal name="delete-program-{{ $item->id }}" class="max-w-sm rounded-md p-10 bg-[#0f172a] border border-white/10">
            <form action="{{ route('admin.program.destroy', $item->id) }}" method="POST">
                ...
                <div class="text-center space-y-6">
                    ...
                    <div class="space-y-2">
                        <h4 class="font-black text-white  tracking-tight">Hapus Program?</h4>
                    ...
                    <div class="flex flex-col gap-3">
                        <button type="submit" class="w-full py-4 bg-red-600 hover:bg-red-700 text-white rounded-md font-black  tracking-widest text-xs border-0 transition-all shadow-lg shadow-red-600/20">Hapus Sekarang</button>
                        <flux:modal.close><button type="button" class="w-full py-4 bg-white/5 text-slate-400 rounded-md font-bold  tracking-widest text-xs hover:bg-white/10 transition-all">Batal</button></flux:modal.close>
                    </div>
                </div>
            </form>
        </flux:modal>
